...
const AddProduct = () => {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setSubcategories(
          p.category_id
            ? (await axios.get(`${BASE_URL}/api/subcategories/getbycategory/${p.category_id}`)).data
            : []
        );
  ...
  const handleFileChange = e => {
    ...
    setPreviewImages(prev => ({ ...prev, [name]: URL.createObjectURL(files[0]) }));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex justify-center p-3 overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-[100vw] p-5 mx-auto min-w-0 overflow-hidden">
        <h2 className="text-xl font-bold text-[#03619E] mb-4 text-center">
          {isEdit ? "Edit Product" : "Add New Product"}
        </h2>

        {message && (
          <p className="text-center font-medium text-green-600 mb-4">{message}</p>
        )}
              ...
              {["image1","image2","image3","image4"].map(key => (
                <div key={key} className="flex flex-col items-center w-full">
                  ...
                  {previewImages[key] && (
                    <img
                      src={
                        previewImages[key].startsWith("http")
                          ? previewImages[key]
                          : `${BASE_URL}/uploads/${previewImages[key].split("/").pop()}`
                      }
